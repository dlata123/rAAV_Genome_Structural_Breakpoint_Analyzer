...
For more samples, add more entries in FILE_REFERENCE_PAIRS.
"""
...
# paths to edit
# ---------------------------------------------------------
# Keep PROJECT_DIR as Path(".") for GitHub. Run the script from the folder
# that contains the input tile file and reference FASTA.
PROJECT_DIR = Path(".")

FILE_REFERENCE_PAIRS = [
    (
        PROJECT_DIR / "example.truncation.tile.zmw.counts",
        PROJECT_DIR / "reference_split.fa",
        PROJECT_DIR / "sample_output_truncation",
    ),
]

# Find RNAfold from PATH. Do not hardcode server-specific paths for GitHub.
...
RESET_OUTPUT_FOLDER = True  # True will recreate output folder each time
...
# ---------------------------------------------------------
# STEP 1: Parse truncation tile-count file
# ---------------------------------------------------------
# The subparser output has tile patterns and counts. For truncation,
# I keep reads where the two payload tiles are on different strands.
# This gives a cleaner table with coordinates and strand information.
# ---------------------------------------------------------
def parse_truncation_tile_file(tile_file: Path, output_folder: Path) -> Path:
    ...
    with tile_file.open("r") as handle:
        for raw_line in handle:
            ...
            # truncation workflow from the notebook keeps opposite-strand payload pairs
...
# ---------------------------------------------------------
# STEP 2: Summarize truncation breakpoint positions
# ---------------------------------------------------------
# For truncation, the notebook uses:
#   plus strand  -> StartA
#   minus strand -> EndB
# The counts are summed at each breakpoint and converted to proportion.
# ---------------------------------------------------------
def summarize_breakpoints(parsed_csv: Path, output_folder: Path) -> tuple[Path, Path]:
    df = pd.read_csv(parsed_csv)

    plus_df = (
        df.loc[df["StrandA"] == "t", ["Count", "StartA"]]
        .groupby("StartA", as_index=False)["Count"]
        .sum()
    )
    plus_total = plus_df["Count"].sum()
    plus_df["Proportion"] = plus_df["Count"] / plus_total if plus_total else 0
    plus_path = output_folder / "PROPORTION_starta.csv"
    plus_df.to_csv(plus_path, index=False)

    minus_df = (
        df.loc[df["StrandB"] == "t", ["Count", "EndB"]]
        .groupby("EndB", as_index=False)["Count"]
        .sum()
    )
    minus_total = minus_df["Count"].sum()
    minus_df["Proportion"] = minus_df["Count"] / minus_total if minus_total else 0
    minus_path = output_folder / "PROPORTION_endb.csv"
...
# ---------------------------------------------------------
# The reference FASTA should contain a record called Payload. If only one
# record is present, I use that record so the example is easier to run.
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# STEP 3B: Write plus and minus payload FASTA files
# ---------------------------------------------------------
# Plus is the original payload. Minus is the reverse complement.
# These sequences are used for local breakpoint window extraction.
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# Helper: read FASTA as sequence text
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# The coordinates are 1-based from the tile file. Python uses 0-based
# indexing, so this converts the coordinate before slicing.
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# STEP 4: Generate breakpoint sequence windows
# ---------------------------------------------------------
# For each breakpoint, this writes different window sizes around the
# truncation coordinate. These are the sequences sent to RNAfold.
# ---------------------------------------------------------
...
# STEP 5: Run RNAfold
# ---------------------------------------------------------
# RNAfold creates .ps files when -p is used. I run it inside a small
# side-folder so those files stay inside plus/minus folders.
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# This keeps windows where MFE structure and centroid structure match,
# then keeps the most negative MFE value after converting to absolute value.
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# STEP 7: Merge plus/minus breakpoint abundance with MFE
# ---------------------------------------------------------
def merge_plus_minus_mfe(output_folder: Path) -> Path:
    plus_counts = pd.read_csv(output_folder / "PROPORTION_starta.csv")
    plus_mfe = pd.read_csv(output_folder / "Drp_duplicates_column_of_Intrst_plus.csv")
    minus_counts = pd.read_csv(output_folder / "PROPORTION_endb.csv")
    minus_mfe = pd.read_csv(output_folder / "Drp_duplicates_column_of_Intrst_minus.csv")

    plus_df = plus_counts.merge(plus_mfe[["StartA", "MFE"]], on="StartA", how="left")
    plus_df["MFE"] = plus_df["MFE"].fillna(0)
    plus_df["Category"] = "Coding / Plus strand"
    plus_df = plus_df.rename(columns={"StartA": "Breakpoint"})[
        ["Breakpoint", "Count", "Proportion", "MFE", "Category"]
    ]

    minus_df = minus_counts.merge(minus_mfe[["EndB", "MFE"]], on="EndB", how="left")
    minus_df["MFE"] = minus_df["MFE"].fillna(0)
    minus_df["Category"] = "Non-Coding / Minus strand"
    minus_df = minus_df.rename(columns={"EndB": "Breakpoint"})[
        ["Breakpoint", "Count", "Proportion", "MFE", "Category"]
    ]
...
# ---------------------------------------------------------
# STEP 9: Nucleotide enrichment analysis
# ---------------------------------------------------------
...
def run_chi_square(output_folder: Path) -> tuple[Path, Path]:
    df = pd.read_csv(output_folder / "nucleotide_counts_by_strand.csv")
...
def plot_breakpoints(output_folder: Path) -> tuple[Path, Path]:
    ...
    for y_col, ylabel, out_name in [
        ...
        for ax, category in zip(axes, categories):
            ...
            ax.set_xlabel("Truncation coordinate")
...
def run_one_sample(tile_file: Path, reference_fasta: Path, output_folder: Path) -> None:
    ...
    print("\n" + "=" * 80)
    print("Running truncation sample")
    print(f"Tile file:       {tile_file}")
    print(f"Reference FASTA: {reference_fasta}")
    print(f"Output folder:   {output_folder}")
    print("=" * 80)

    parsed_csv = parse_truncation_tile_file(tile_file, output_folder)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Truncation RNAfold analysis")
    parser.add_argument(
        "--sample",
        nargs=3,
        action="append",
    ...
    parser.add_argument(
        "--rnafold",
        default=RNAFOLD_CMD,
        help="RNAfold command/path. Default: RNAfold found from PATH.",
    )
    parser.add_argument(
        "--keep-output-folder",
        action="store_true",
